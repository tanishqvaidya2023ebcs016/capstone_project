"""
Ranked Crawler — Algorithm 
Uses ResilientQueueStub to automatically failover 
between Mac and Oracle Queue Servers if one goes down.
"""
...
class MetricsReporter:
    """Reports per-machine crawl throughput + RTT to the dashboard every 5s."""
    def __init__(self, crawler_id, queue_stub, file_stub, dashboard_url, logger):
        ...
        self.q_rtts        = deque(maxlen=120)
        self.f_rtts        = deque(maxlen=120)

        self._lock       = threading.Lock()
        self._crawl_ts   = deque()
        self._store_ts   = deque()
        ...
        if self.dashboard_url:
            ...
            self.logger.info(f"📡 Reporting metrics to {self.dashboard_url}")
        else:
            self.logger.info("📡 DASHBOARD_URL not set — this machine won't appear on Live Monitor")

    def record_crawl(self):
        now = time.time()
        with self._lock:
            self._crawl_ts.append(now)
            self.total_crawled += 1
            cutoff = now - 300
            while self._crawl_ts and self._crawl_ts[0] < cutoff:
                self._crawl_ts.popleft()

    def record_store(self):
        now = time.time()
        with self._lock:
            self._store_ts.append(now)
            self.total_stored += 1
            cutoff = now - 300
            while self._store_ts and self._store_ts[0] < cutoff:
                self._store_ts.popleft()

    def _rolling_lpm(self, ts_deque, window_sec=60):
        cutoff = time.time() - window_sec
        count  = sum(1 for t in ts_deque if t >= cutoff)
        return round(count / (window_sec / 60), 1)

    def _probe(self):
        try:
            t0 = time.perf_counter()
            self.queue_stub.GetStats(crawler_pb2.GetStatsRequest(), timeout=3)
            self.q_rtts.append((time.perf_counter() - t0) * 1000)
        except Exception:
        ...
        try:
            ...
            self.f_rtts.append((time.perf_counter() - t0) * 1000)
        except Exception:
            self.f_rtts.append(-1)
    ...
    def _rtt_stats(rtts):
        ...
        if not v:
            return {'current': -1, 'avg': -1, 'p95': -1, 'p99': -1, 'min': -1, 'max': -1}
        s = sorted(v); n = len(s)
        return {
            'current': round(v[-1], 1),
            'avg':     round(sum(s) / n, 1),
    ...
    def _report_loop(self):
        while True:
            ...
            try:
                with self._lock:
                    lpm_crawled = self._rolling_lpm(self._crawl_ts, 60)
                    lpm_stored  = self._rolling_lpm(self._store_ts,  60)
                    total_c     = self.total_crawled
                    total_s     = self.total_stored
                payload = {
                    'crawler_id':    self.crawler_id,
                    'timestamp':     time.time(),
                    'queue_rtt':     self._rtt_stats(list(self.q_rtts)),
                    'file_rtt':      self._rtt_stats(list(self.f_rtts)),
                    'uptime_sec':    time.time() - self.start_time,
                    'lpm_crawled':   lpm_crawled,
                    'lpm_stored':    lpm_stored,
                    'total_crawled': total_c,
                    'total_stored':  total_s,
                }
                requests.post(
                ...
                self.logger.debug(f"Metrics report failed: {e}")


# ============================================================
# RESILIENT QUEUE STUB (HA Failover)
# ============================================================
class ResilientQueueStub:
    """
    Wraps multiple grpc_server.py addresses (primary + fallback) and
    automatically fails over when the primary is unreachable.
    """
    FAILOVER_THRESHOLD = 3   # consecutive failures before switching

    def __init__(self, servers: list, logger):
        """servers: list of 'host:port' strings, in priority order."""
        self._logger   = logger
        self._servers  = servers
        self._channels = [grpc.insecure_channel(s) for s in servers]
        self._stubs    = [crawler_pb2_grpc.QueueServiceStub(ch) for ch in self._channels]
        self._active   = 0
        self._fail_cnt = 0
        logger.info(f"Queue servers (in failover order): {servers}")

    def _stub(self):
        return self._stubs[self._active]
    ...
    def _on_failure(self, err):
        self._fail_cnt += 1
        if self._fail_cnt >= self.FAILOVER_THRESHOLD and len(self._stubs) > 1:
            old = self._servers[self._active]
            self._active  = (self._active + 1) % len(self._stubs)
            self._fail_cnt = 0
            self._logger.warning(
                f"⚡ Queue server failover: {old} → {self._servers[self._active]} "
                f"(after {self.FAILOVER_THRESHOLD} consecutive failures)"
            )

    def GetNextURL(self, request, timeout=10):
        try:
            r = self._stub().GetNextURL(request, timeout=timeout)
            self._on_success()
            return r
    ...
    def AddURLs(self, request, timeout=10):
        try:
            r = self._stub().AddURLs(request, timeout=timeout)
            self._on_success()
            return r
        except grpc.RpcError as e:
            self._on_failure(e)
    ...
    def MarkVisited(self, request, timeout=10):
        try:
            r = self._stub().MarkVisited(request, timeout=timeout)
            self._on_success()
            return r
        except grpc.RpcError as e:
            self._on_failure(e)
    ...
    def GetStats(self, request, timeout=10):
        try:
            r = self._stub().GetStats(request, timeout=timeout)
            self._on_success()
            return r
        except grpc.RpcError as e:
            self._on_failure(e)
    ...
    def SeedURLs(self, request, timeout=10):
        try:
            r = self._stub().SeedURLs(request, timeout=timeout)
            self._on_success()
            return r
        except grpc.RpcError as e:
            self._on_failure(e)
...
# ============================================================
class RankedCrawler:
    CODE_VERSION = "ha-sentinel-v5"

    def __init__(self, crawler_id, queue_servers: list, file_server, max_workers=3):
        self.crawler_id = crawler_id
        self.logger = logging.getLogger(crawler_id)
        self.logger.info(
            f"🔖 crawler.py CODE_VERSION={self.CODE_VERSION} | "
            f"queue_servers={queue_servers} | file_server={file_server}"
        )
        ...
        # Resilient queue stub: tries each server in order, promotes next on failure
        self.queue_stub = ResilientQueueStub(queue_servers, self.logger)
    ...
    def crawl_url(self, url):
        ...
        try:
            ...
            self._store_to_grpc(url, score_data)
            self.stats['stored'] += 1
    ...
    def _store_to_grpc(self, url, score_data):
        try:
            resp = self.file_stub.StoreLink(
                crawler_pb2.StoreLinkRequest(
                    url=url,
                    crawler_id=self.crawler_id,
            ...
            if resp.success:
                ...
                self.logger.warning(f"⚠️ file_server reported failure storing: {url}")
        except TypeError as e:
            self.logger.error(
                f"❌ StoreLink TypeError — crawler_pb2.py probably missing score/domain "
                f"fields. Re-copy crawler_pb2.py+crawler_pb2_grpc.py. Error: {e}"
            )
        except grpc.RpcError as e:
            self.logger.error(
                f"❌ Cannot reach file_server for {url} — "
                f"code={e.code()} details={e.details()}"
            )
        except Exception as e:
            self.logger.error(f"❌ Unexpected _store_to_grpc error for {url}: {e}")

    def run(self):
        self.logger.info(f"🚀 Enhanced Ranked Crawler ({self.crawler_id}) running FOREVER...")
        consecutive_empty = 0
            ...
            while True:
                ...
                for _ in range(self.max_workers):
                    try:
                        ...
                        self.logger.error(f"Failed to get next URL: {e}")
                        consecutive_empty += 1
                        time.sleep(2)
                        continue

                if not batch_urls:
                    backoff = min(15, max(1, consecutive_empty))
                    self.logger.info(f"Queue empty/failure. Waiting {backoff}s... (fail count: {consecutive_empty})")
                    time.sleep(backoff)
                    continue
                ...
                while len(futures) >= self.max_workers * 2:
                    ...
                    for f in done:
                        try:
                            ...
                            self.logger.error(f"Unhandled error in crawl thread: {e}")
...
# ============================================================
def main():
    # Read comma-separated list for HA Failover
    queue_servers_raw = os.environ.get('QUEUE_SERVERS', 'localhost:50051')
    queue_servers = [s.strip() for s in queue_servers_raw.split(',')]
    ...
    # Note: MAX_URLS is now completely ignored; crawler runs forever.
    crawler = RankedCrawler(crawler_id, queue_servers, file_server, max_workers=workers)
    try:
        if should_seed:
