...
Measures how fast distributed nodes are working together.
- Connects to Redis Queue Server via gRPC
- Polls visited count every second
- Calculates rolling LPM for 1min, 5min, 10min windows
- Shows per-crawler breakdown

Run on Mac:
    QUEUE_SERVER=localhost:50051 python monitor/lpm_monitor.py

Run on Windows:
    set QUEUE_SERVER=100.x.x.1:50051
    python monitor/lpm_monitor.py
"""
